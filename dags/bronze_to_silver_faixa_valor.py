...
def enviar_produtos_faixa_valor_para_s3():
    diretorio_parquet = "/tmp/produtos_faixa_valor_temp"
    bucket_destino = "silver-autoglass"
    nome_arquivo_s3 = "produtos_faixa_valor.parquet"
...
with DAG(
    dag_id='enviar_produtos_faixa_valor',
    default_args=default_args,
    schedule_interval=None,
    catchup=False,
    tags=['s3', 'faixa_valor', 'silver']
) as dag:

    enviar_task = PythonOperator(
        task_id='enviar_parquet_s3',
        python_callable=enviar_produtos_faixa_valor_para_s3
    )
